...
void testCouncilRoom() {
	...
	printf("----- Unit Testing for council_room card effect. ----- \n");
	...
	for (int i = 0; i < numPlayer; i++)
	{
		...
		{
			playerBool = 1;
			printf("Current Player Test Failed. Expected  = %i; Actual = %i \n", testG.whoseTurn, currentPlayer3);
		}
		...
		{
			positionBool = 1;
			printf("Current Hand Position Test Failed. Expected  = %i; Actual = %i \n", handPos, handPos5);
		}

	}
	//Compare actual vs expected results and post
	if ((playerBool)&&(positionBool))
		printf("All unit tests passed for council_room. \n");
	else
