...
VILLAGE: Sequence of opertations
Begin turn
1. player draws 1 card	-> +1 cards to handCount
2. player increases 2 actions
...
void testVillage(int p, struct gameState *post)
{
	int r;
	int bonus = 0;
	...
	//village takes 1 card from the deck, actions increased by 2, and discards 1
...
int main()
{
	...
	printf("--- Testing cardEffect for village. --- \n");
	printf("RANDOM TESTS. \n");
	srand(time(NULL));
	...
	for (int n = 0; n < MAX_ITERATIONS; n++)
	{
		...
		G.numActions = floor(rand() % 8); //if all 5 cards were village- could have 8 + 2 new actions

		//Use the random values to test village
		testVillage(p, &G);
	}
	return 0;
}
